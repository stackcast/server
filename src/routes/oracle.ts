...
import { fetchCallReadOnlyFunction, cvToValue, bufferCV, principalCV } from '@stacks/transactions';
...
const NETWORK_TYPE = (process.env.STACKS_NETWORK as 'mainnet' | 'testnet' | 'devnet') || 'devnet';
const NETWORK = getNetwork(NETWORK_TYPE);

// Parse contract address from environment or use default
const ORACLE_ADDRESS = process.env.OPTIMISTIC_ORACLE_ADDRESS || 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.optimistic-oracle';
...
oracleRoutes.get('/questions/:questionId', async (req: Request, res: Response) => {
  try {
...
oracleRoutes.get('/questions/:questionId/vote/:address', async (req: Request, res: Response) => {
  try {
...
oracleRoutes.get('/questions/:questionId/resolved', async (req: Request, res: Response) => {
  try {
    const { questionId } = req.params;
